...
function App() {
    ...
    fetchSiteSettings()
      .then((settings) =>
        setSiteSettings({
          ...DEFAULT_SETTINGS,
          ...settings,
          popup: { ...DEFAULT_POPUP, ...(settings.popup || {}) },
  ...
  return (
    <div className={`app-container ${isMobile ? 'mobile' : ''}`} data-theme={siteSettings.theme}>
        ...
        <div className="header-content">
          ...
          >
            <h1>
              ...
              River Watch
              {themeMark && themeMark !== '⚓' ? (
                <span className="theme-header-mark" aria-hidden="true">
                  {themeMark}
                </span>
              ) : null}
            </h1>
        ...
        {page === 'feedback' ? (
          ...
          <>
            {publishedAnnouncements.length > 0 && (
              <div className="announcement-row" aria-label="Site announcements">
              ...
              <div className="stats-card">
                ...
                <div className="stats-info">
                  ...
                  <div className="stats-label">
                    Vancouver{' '}
                    <span className="stats-mode-tag">
                      {movementType === 'tie-ups' ? 'Tie Ups' : 'Let Go'}
                    </span>
                  </div>
                </div>
              </div>
              ...
              <div className="table-wrapper">
                {searchFilteredVessels.length > 0 ? (
                  ...
                  <div
                    ...
                    style={{ padding: '4rem 2rem', textAlign: 'center', color: '#94a3b8' }}
                  >
                    <div style={{ fontSize: '2rem', marginBottom: '1rem' }}>🚢</div>
                    <div>
                      No {movementType === 'tie-ups' ? 'tie ups' : 'let gos'} match your search "
                      {searchTerm}" in {activePort}.
                    </div>
                  </div>
                )}
              </div>
      ...
      <UpdateNotice />
      {page === 'dashboard' && <PopupAnnouncementModal popup={siteSettings.popup} />}
    </div>
  );
}
